...
function sendRegistration(user){
    var contents = ejs.render(templates.registration, {
        ...
        html = ejs.render(templates.mail, {
            ...
            header: util.format("%s, tere tulemast kasutama teenust %s!", user.name, config.title),
            contents: contents
        }),
        mailOptions;
    ...
    mailOptions = {
        ...
        subject: util.format("%s, tere tulemast kasutama teenust %s!", user.name, config.title),
        html: html
    };
...
function sendResetLink(user, resetToken){
    var contents = ejs.render(templates.resetLink, {
        ...
        html = ejs.render(templates.mail, {
            ...
            title: config.title,
            twitter: config.twitter,
            header: util.format("Uue parooli genereerimine %s teenuse kasutamiseks", config.title),
            contents: contents
        }),
        mailOptions;
    ...
    mailOptions = {
        from: config.mail.from,
        to: "\"" + user.name.replace(/"/g, "") + "\" <" + user.id + ">",
        subject: util.format("Uue parooli genereerimine %s teenuse kasutamiseks", config.title),
        html: html
    };
...
function sendPassword(user, password){
    var contents = ejs.render(templates.sendPassword, {
        ...
        html = ejs.render(templates.mail, {
            ...
            title: config.title,
            twitter: config.twitter,
            header: util.format("%s, sinu parool on uuendatud!", user.name),
            contents: contents
        }),
        mailOptions;
    ...
    mailOptions = {
        from: config.mail.from,
        to: "\"" + user.name.replace(/"/g, "") + "\" <" + user.id + ">",
        subject: util.format("%s, sinu parool on uuendatud!", user.name),
        html: html
    };
